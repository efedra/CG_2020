...
void initShader()
{
	//! Исходный код шейдеров  - презентация 
	const char* vsSource =
	...
	///! Вытягиваем ID атрибута из собранной программы   - из пдф
	...
	///Вытягиваем ID юниформ матрицы проекции -презентация
	attr_name = "matrix";
	Unif_matrix = glGetUniformLocation(Program, attr_name);

	checkOpenGLerror();
}
//! Инициализация VBO - презентация
void initVBO()
{
	GLfloat vertices[] =
	{
...
3, 1, 2
	...
	//Передаем цвет в буфер- презентация
...
void resizeWindow(int width, int height)
{
	...
	Projection = glm::perspective(45.0f, (GLfloat) width / (GLfloat) height, 1.0f, 200.0f);
}



double a = 0;// для вращения
...
void render()
{
	...
	//
	glm::mat4 View = glm::lookAt(glm::vec3(4, 3, 3), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0));// видовая матрица
...
//общий для цветов и их вершин - презентация
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, VBO_element);
