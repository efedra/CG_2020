...
void text()
{
	...
	// Texture 1
	// ====================
	glGenTextures(1, &texture1); // создали идентификатор
	glBindTexture(GL_TEXTURE_2D, texture1); // привязали
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); // повторяем по оси x
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // повторяем по оси y

	
	int width, height;
	unsigned char* image = SOIL_load_image("2.jpg", &width, &height, 0, SOIL_LOAD_RGB);
	...
	// загружаем, создаем текстуры  и генерируем мипмепы
	image = SOIL_load_image("kit.png", &width, &height, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	glGenerateMipmap(GL_TEXTURE_2D);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
